...
along a ring homomorphism might not be an ideal [this is right! The image of Z under Z→Z[x] isn't an ideal], you cannot in general add two
...
begin
  split, intro h, split;
    ...
    apply inter_subset_left, apply inter_subset_right,
  exact λh, inter_mem_sets h.1 h.2
end
...
{ sets := {S : set α | X ⊆ S},
  univ_sets := begin
    intros _ _, triv -- or apply mem_univ
  end,
  sets_of_superset := begin
    intros S T h1 h2, exact subset.trans h1 h2
  end,
  inter_sets := begin
    intros S T hS hT, exact subset_inter hS hT
  end }
...
begin
  rw le_def, split,
    intro h, exact h T (subset.refl T),
    intros h X hX, exact subset.trans h hX
end
...
begin
  rw le_def, split,
    intro h, exact h S (λ_,id /- or subset.refl S -/),
    intros h T hT, exact mem_sets_of_superset h hT
end
...
{ sets := {X : set L | ∃ x : L, ∀ y, x ≤ y → y ∈ X},
  univ_sets := begin
    use e, intros y _, triv
  end,
  sets_of_superset := begin
    rintros S T ⟨x,hS⟩ hc, use x, intros y hy, exact hc (hS y hy)
  end,
  inter_sets := begin
    rintros S T ⟨x,hS⟩ ⟨z,hT⟩, use max x z, intros y hy,
    split, apply hS, exact le_trans (le_max_left _ _) hy,
           apply hT, exact le_trans (le_max_right _ _) hy
  end }
...
{ sets := { S : set α | (Sᶜ).finite },
  univ_sets := begin
    dsimp, rw compl_univ, exact finite_empty
  end,
  sets_of_superset := begin
    intros S T hS hc, rw ← compl_subset_compl at hc,
    exact finite.subset hS hc
  end,
  inter_sets := begin
    intros S T hS hT, dsimp, rw compl_inter, exact finite.union hS hT
  end }
...
thanks to Yakov Pechersky on Zulip for coming up with the one-liner)
-/

lemma infinite_of_finite_compl {α : Type} [infinite α] {s : set α}
  (hs : sᶜ.finite) : s.infinite :=
λ h, infinite_univ (by simpa using hs.union h)
...
begin
  intros S hS, by_cases h : S.nonempty,
  { cases h with x hx,
    have : {x}ᶜ ∈ cofinite ℕ,
      rw [mem_cofinite, compl_compl], apply finite_singleton,
    rw hS at this, exact this hx (mem_singleton x)
    /-have : {y : ℕ | y < x.succ}ᶜ ∈ cofinite ℕ,
      rw [mem_cofinite, compl_compl],
      exact ⟨fin.fintype x.succ⟩,
    rw hS at this,
    apply this hx,
    apply nat.le_refl-/ },
  { have : ∅ ∈ 𝓟 S,
      rw not_nonempty_iff_eq_empty at h,
      rw h, apply subset.refl,
    revert this,
    rw [←hS, mem_cofinite, compl_empty],
    exact infinite_univ }
end
